...
| 指标 | 数值 | 基础门槛 | 追加门槛 |
| --- | --- | --- | --- |
| 平均 val MSE (96/192/336/720) | **0.003632** | < 0.01 | < 0.006 (+5) / < 0.005 (+10) |
| Demo set MSE | **0.003030** | < 0.01 | < 0.006 (+5) / < 0.005 (+10) |
...
> 现场测试数据按作业 PDF: 形状 `(250, 96, 100)`，对应输出 `pred_<L>.npy` 形状 `(250, L, 100)`，L ∈ {96,192,336,720}。22 模型集成在 CPU 上 250 个样本 ~3-5 分钟，GPU ~10 秒。
...
> **假设`test_x.npy`，形状 `(N, 96, 100)`，每行是一个长度 96 的输入窗口。需要为每个窗口分别预测未来 96/192/336/720 步。
